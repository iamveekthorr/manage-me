...
- **Links**: Represents worker connections (references `user_id` and `manager_id`)
  - Contains employment details: `job_type`, `expected_salary`, `overtime_pay`, `start_date`, `end_date`
  - Tracks active status via `is_active` (u8: 0=false, 1=true)
...
1. **Type Inconsistencies**
   - `users_model.rs` uses `String` for IDs
   - Other models (`links.rs`, `managers.rs`, `payouts.rs`) use `u32` for IDs
   - **Decision:** Standardize on `String` (TEXT in PostgreSQL) for all IDs for flexibility and UUID support
